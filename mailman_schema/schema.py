...
class BaseMessage(message.Message):
    ...
    def __str__(self):
        ...
        return "From: {send}\nTo: {rec}\nSubject: {subj}\n{body}\n".format(
            send=self.sender, rec=self.receiver, subj=self.subject, body=self.email_body
        )
...
class MessageV4(BaseMessage):

    body_schema = {
        'id': 'n/a',
        '$schema': 'http://json-schema.org/draft-04/schema#',
        'description': 'A new message schema made by the package creator',
        'type': 'object',
        'required': ['sender', 'receiver', 'subject', 'body'],
        'properties': {
            'sender': {'type': 'string'},
            'receiver': {'tupe': 'string'},
            'subject': {'type': 'string'},
            'body': {'type': 'string'},
            'replying-to': {'type': 'string'},
            'time-sent': {'type': 'string'},
            'archived-at': {'type': 'string'},
        },
    }
